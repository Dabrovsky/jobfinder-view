...
function Header(props) {
  ...
  const handleKeywordsChange = (input) => {
    const keywords = input.target.value.replace(/[ ,.-]+/g, ",");
    ...
    if (keywords.length) {
      document.querySelector(".clear-keywords-button").classList.remove("hidden");
    } else {
      document.querySelector(".clear-keywords-button").classList.add("hidden");
    }
  };

  const clearSearchKeyowrds = () => {
    setFilters(prevState => ({
      ...prevState,
      keywords: ""
    }));
  ...
  const sendForm = () => {
    const { page, ...currentFilters } = filters; // eslint-disable-line no-unused-vars
      ...
      <div className="container px-4 md:px-0 py-12 mx-auto">
        ...
        <form className="flex flex-col w-full mb-4 md:flex-row">
          <div className="relative md:w-7/12 border-b-2 md:border-b-0 md:border-r-2 input-container">
            ...
            <input name="keywords" onInput={handleKeywordsChange} onKeyDown={handleSubmit} value={filters.keywords?.toString().replace(/[,]+/g, ", ")} placeholder="keywords: ex. cloud, admin" className="input px-9 py-3 text-sm rounded-none md:rounded-l"></input>
            <button type="button" onClick={clearSearchKeyowrds} className="clear-keywords-button absolute top-1/2 transform -translate-y-1/2 right-3 hidden text-blue-400" aria-label="Clear Selected">
              <svg width="24" height="24" fill="none" stroke="currentColor" strokeWidth="2" className="stroke-gray-400">
